...
function reverseEachWord(string, yesNo = false) {
    let reversedWords = string.split(' ').map(function(val) {
        return val.split('').reverse().join('');
    })
    return (yesNo ? reversedWords.reverse().join(' ') : reversedWords.join(' '));
}
...
function wordCounter(sentence) {
    let arra = sentence.split(' ');
    let obj = {};
    arra.forEach(function(val) {
        obj[val] = (obj[val] != undefined ? ++obj[val] : 1);
    });
    return obj;
};
...
function uniqueElements(arr) {
  let uniqeArr = [];
  uniqeArr = arr.filter(function(val, ind) {
