 ...
 *
 * */
let openBra =  ["(", "[", "{"]; //набор открывающихся скобок
let closeBra = [")", "]", "}"]; //набор закрывающихся скобок (индексы должны соответствовать)
function validBraces(str) {
  console.log(str);
  let opened = [];
  for (let i = 0; i < str.length; i++) {
    if (openBra.indexOf(str[i]) > -1) {// проверка есть ли в массиве открытых скобок
      opened.push(str[i]); // если есть добавляем в массив открытые скобки
    }
    if (closeBra.indexOf(str[i]) > -1) {// проверка есть ли в массиве закрытых скобок
